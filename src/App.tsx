...
import { subscribeToSiteConfig, subscribeToVentures, subscribeToInsights } from "./lib/firebase";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubInsights = subscribeToInsights((liveInsights) => {
      if (liveInsights && liveInsights.length > 0) {
        applySiteConfig({ insights: liveInsights });
      }
    });
  ...
  const handleBookClick = (serviceName?: string) => {
    ...
    setPreselectedService(serviceName || defaultService);
