...
export async function verifySmtpConnection(config: SmtpConfig): Promise<{
  ...
  try {
    ...
    let userFriendlyMessage = `SMTP verification failed: ${errMsg}`;
    
    if (err?.code === "ECONNREFUSED" || errMsg.includes("ECONNREFUSED")) {
      if (config.host.includes("metaspaceconsulting.com") || errMsg.includes("212.123.41.108")) {
        userFriendlyMessage = `Connection Refused on ${config.host}:${config.port} (IP: 212.123.41.108). The domain 'metaspaceconsulting.com' currently resolves to the registrar's parking web server, which does not run an SMTP mail daemon. To send emails immediately, please select a working SMTP provider such as Gmail / Google Workspace (smtp.gmail.com:465 with a Google App Password), Brevo (smtp-relay.brevo.com:587), or your hosting provider's direct mail server (e.g. mail.privateemail.com).`;
      } else {
        userFriendlyMessage = `Connection Refused on ${config.host}:${config.port}. No SMTP mail service is listening on port ${config.port} at this host. Please verify the hostname and try port 465 (SSL/TLS) or 587 (STARTTLS).`;
      }
    } else if (err?.code === "EAUTH" || errMsg.includes("Invalid login") || errMsg.includes("Username and Password not accepted")) {
      userFriendlyMessage = `Authentication Failed on ${config.host}: The username or password was rejected. For Gmail/Google Workspace, you must generate a 16-character Google App Password under Google Account > Security > App Passwords (standard account passwords are blocked by Google).`;
    } else if (err?.code === "ETIMEDOUT" || errMsg.includes("ETIMEDOUT")) {
      userFriendlyMessage = `Connection Timed Out on ${config.host}:${config.port}. The server took too long to respond. Check if the port is open and reachable.`;
    }
