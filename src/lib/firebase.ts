...
/**
 * Real-time subscription to Insights collection in Firestore
 */
export function subscribeToInsights(callback: (insights: any[]) => void): () => void {
  try {
    const iColRef = collection(db, "insights");
    return onSnapshot(iColRef, (snap) => {
      if (!snap.empty) {
        const list = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        callback(list);
      }
    }, (err) => {
      console.warn("Real-time insights subscription warning:", err);
    });
  } catch (err) {
    console.warn("Could not attach real-time insights listener:", err);
    return () => {};
  }
}
...
/**
 * TAB Insights: Save Insights & Articles into Cloud Database
 */
export async function saveInsightsToCloudDatabase(
  insights: any[],
  extra?: { username?: string }
): Promise<CloudSaveNotice> {
  const now = new Date();
  ...
  try {
    // 1. Commit array to site_config/global
    const docRef = doc(db, "site_config", "global");
    await setDoc(docRef, {
      insights,
      insightsLastSavedAt: now.toISOString(),
      savedBy: extra?.username || "admin"
    }, { merge: true });

    // 2. Commit each post to individual documents in 'insights' collection
    for (const post of insights) {
      if (post.id) {
        const pRef = doc(db, "insights", String(post.id));
        await setDoc(pRef, {
          ...post,
          lastUpdatedAt: now.toISOString()
        }, { merge: true });
    ...
    if (!readBack.exists()) {
      return {
        saved: false,
        message: `Notice: Insights updates could not be verified in Cloud Database '${targetDb}'.`,
        timestamp: timestampStr,
        verified: false,
        databaseId: targetDb,
        error: "Read-back verification failed."
    ...
    return {
      saved: true,
      message: `Data was indeed saved successfully to Cloud Database: ${insights.length} Insights Articles verified in collections 'insights' and 'site_config/global'.`,
      timestamp: timestampStr,
      verified: true,
      databaseId: targetDb,
      docPath: "insights & site_config/global"
    };
  } catch (err: any) {
