...
interface ConsultationFormProps {
  ...
  availableServices?: (string | { title: string })[];
...
export default function ConsultationForm({ 
  ...
  const dynamicServices = availableServices && availableServices.length > 0
    ? availableServices.map(s => typeof s === "string" ? s : s.title).filter(Boolean)
    : [
  ...
  const defaultService = preselectedService || (dynamicServices.length > 0 ? dynamicServices[0] : "Venture Design Studio");
  ...
  // Keep selected service in sync with dynamic services or preselected service
  useEffect(() => {
    if (preselectedService && preselectedService.trim() !== "") {
      setService(preselectedService);
    } else if (dynamicServices.length > 0) {
      setService(dynamicServices[0]);
    }
  }, [preselectedService, JSON.stringify(dynamicServices)]);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex flex-col space-y-1.5">
                ...
                <select
                  ...
                  className="px-3.5 py-2.5 text-xs bg-gray-50 border border-gray-200 focus:border-blue-900 rounded-lg focus:outline-none focus:bg-white transition"
                >
                  {service && !dynamicServices.includes(service) && (
                    <option value={service}>{service}</option>
                  )}
                  {dynamicServices.map((ser, idx) => (
